...
const String Main::AccelGraph = R"rawliteral(
<!DOCTYPE html>
<html>
...
void Main::HTTPInit()
{
  ...
  server[0].on("/accel", HTTP_GET, [](AsyncWebServerRequest *request)
               { request->send_P(200, "text/html", AccelGraph.c_str()); });
  ...
  server[0].on("/api/json", HTTP_GET, [](AsyncWebServerRequest *request)
               {
                ...
                delete[] text; });
  server[0].on("/api/acceldata", HTTP_GET, [](AsyncWebServerRequest *request)
               {
                 String str;
